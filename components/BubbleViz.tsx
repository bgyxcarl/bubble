...
const BubbleViz: React.FC<BubbleVizProps> = ({ data, activeType, setActiveType, onAddData, theme, baseAddresses }) => {
                ...
                <MotionDiv drag dragControls={controlsDragControls} dragMomentum={false} initial={{ x: -300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -300, opacity: 0 }} className={`absolute top-20 left-4 w-72 ${panelBg} border-4 ${panelBorder} neo-shadow-lg z-20 flex flex-col max-h-[calc(100vh-100px)]`}>
                    ...
                    <div className="p-4 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                             ...
                             <div className={`p-3 border-2 ${panelBorder} ${theme === 'light' ? 'bg-gray-50' : 'bg-gray-900'}`}>
                                ...
                                <div className={`max-h-32 overflow-y-auto custom-scrollbar border p-1 space-y-1 ${theme === 'light' ? 'bg-white border-gray-300' : 'bg-black border-gray-700'}`}>
                                    {availableTokens.map(token => (
                                        <div key={token} onClick={() => toggleToken(token)} className={`flex items-center gap-2 p-1.5 cursor-pointer text-xs font-mono font-bold transition-colors ${theme === 'light' ? 'hover:bg-blue-50 text-slate-900' : 'hover:bg-blue-900/20 text-gray-200'}`}>
                        ...
                        <div>
                            ...
                            <input type="range" min="0" max="1000" step="10" className="w-full accent-black h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" value={dustThreshold} onChange={(e) => setDustThreshold(parseFloat(e.target.value))} />
                            <div className="text-right text-xs font-mono font-bold mt-1">{dustThreshold}</div>
                        </div>
                    </div>
                </MotionDiv>
            )}
